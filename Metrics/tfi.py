...
class TFI:
    ...
    def spikes2images(self, spikes, max_search_half_window=20):
        '''
        Texture From Interval (TFI) 
        reconstruct spikes into images
        
        Input：
        spikes: T x H x W numpy tensor
        max_search_half_window: maximum number of reference

        Output：
        ImageMatrix: T' x H x W  numpy tensor, T' = T - (2 x max_search_half_window)
