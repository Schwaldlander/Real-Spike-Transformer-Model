...
parser.add_argument('--pretrained', '-prt', type=str, default='/root/RSFM/ckpt_2023-05-2119:17:20/best.pth')
    ...
    with torch.no_grad():
        ...
        for scene, cur_test_set in test_datasets.items():
            ...
            for ww, data in enumerate(cur_test_loader, 0):
                ...
                #predImg = (predImg + 1.) / 2. * 255. ####这里是一个关键节点！
        ...
        for pre, gt in zip(pres_a, gts_a):
            ...
            deb = (255 - np.asarray(pre) ).astype(np.uint8)
            
            concatImg = np.concatenate([pre[0][0],  deb[0][0],gt[0][0]], axis=1)
